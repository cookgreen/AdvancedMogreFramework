...
namespace org.ogre.framework.app
{
    ...
    public class SinbadCharacter
	{
		...
		private StateMachine<SinbadCharacter> machine;
	
	
	    public SinbadCharacter(Camera cam)
	    {
	        setupBody(cam.SceneManager);
			mCamera = cam;
			machine = new StateMachine<SinbadCharacter>(this);
	        ...
	        for (int i = 0; i < NUM_ANIMS; i++)
		    {
			    if (mFadingIn[i])
			    {
				    ...
				    float newWeight = mAnims[i].Weight + deltaTime * ANIM_FADE_SPEED;
						mAnims[i].Weight = OgreFramework.Instance.Clamp(newWeight, 0, 1);
